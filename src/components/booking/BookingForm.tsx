...
const BookingForm = ({ tour, onCancel }: BookingFormProps) => {
  ...
  const calculateTotal = () => {
    const adultTotal = tour.price_adult * formData.adultsCount;
    const childTotal = tour.price_child * formData.childrenCount;
    const infantTotal = tour.price_infant * formData.infantsCount;
        ...
        <CardContent className="pt-4">
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span>₹{(tour.price_adult * formData.adultsCount).toLocaleString()}</span>
            </div>
            {formData.childrenCount > 0 && (
              <div className="flex justify-between">
                <span>Children ({formData.childrenCount})</span>
                <span>₹{(tour.price_child * formData.childrenCount).toLocaleString()}</span>
              </div>
            )}
            {formData.infantsCount > 0 && (
              <div className="flex justify-between">
                <span>Infants ({formData.infantsCount})</span>
                <span>₹{(tour.price_infant * formData.infantsCount).toLocaleString()}</span>
              </div>
            )}
            <div className="flex justify-between">
              <span>Subtotal</span>
